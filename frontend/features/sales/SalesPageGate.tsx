...
type SalesPageGateProps = {
  permission: string | readonly string[];
  children: ReactNode;
};

export function SalesPageGate({ permission, children }: SalesPageGateProps) {
    ...
    fetchAndStorePermissions()
      .then((permissions) => {
        if (!hasPermission(permissions, permission)) {
          setError('You do not have permission to access this sales page.');
          setAllowed(false);
          return;
        }
        setAllowed(true);
  ...
  if (loading) return <LoadingState title="Checking sales access" />;
