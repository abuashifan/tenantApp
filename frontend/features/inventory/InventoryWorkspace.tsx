    ...
    <AppShell>
      <InventoryPageGate permission={INVENTORY_NAV_ITEMS.map((item) => item.permission)}>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((item) => <Link key={item.href} href={item.href} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:shadow"><h2 className="font-semibold text-slate-950">{item.label}</h2><p className="mt-2 text-sm text-slate-600">{item.description}</p></Link>)}
        </div>
      </InventoryPageGate>
    </AppShell>
  );
