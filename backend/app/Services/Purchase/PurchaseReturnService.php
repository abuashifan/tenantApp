...
class PurchaseReturnService
{
    ...
    public function void(PurchaseReturn $return, ?string $reason = null): PurchaseReturn { $return->status = 'void'; $return->voided_by = auth()->id(); $return->voided_at = now(); $return->void_reason = $reason; $return->save(); return $return->refresh(); }
    ...
    private function guardDate(string $date): void { $check = $this->dateGuardService->check($date, 'post', 'purchase'); if ($check->denied()) { $arr = $check->toArray(); throw ApiException::make((string) $arr['code'], (string) $arr['message'], 422, (array) $arr['reasons'], (array) $arr['meta']); } }
}
