...
export const backendResourceFormConfigs: Record<string, ResourceFormConfig> = {
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { key: 'is_customer', label: 'Customer', kind: 'checkbox' },
          { key: 'is_supplier', label: 'Supplier', kind: 'checkbox' },
          { key: 'is_employee', label: 'Employee', kind: 'checkbox' },
  ...
  '/inventory/stock-movements': {
    ...
    lineItems: { key: 'lines', title: 'Movement Lines', minRows: 1, columns: [{ key: 'product_id', label: 'Product ID', type: 'number' }, { key: 'warehouse_id', label: 'Warehouse ID', type: 'number' }, { key: 'quantity', label: 'Quantity', type: 'number' }, { key: 'unit_cost', label: 'Unit Cost', type: 'money' }], defaultRow: { product_id: null, warehouse_id: null, quantity: 1, unit_cost: 0 } },
  ...
  '/inventory/stock-adjustments': {
    ...
    sections: [{ title: 'Adjustment Details', fields: [{ key: 'adjustment_date', label: 'Adjustment Date', kind: 'date', required: true }, { key: 'warehouse_id', label: 'Warehouse ID', kind: 'number', required: true }, { key: 'reason', label: 'Reason', kind: 'textarea' }] }],
    lineItems: { key: 'lines', title: 'Adjustment Lines', minRows: 1, columns: [{ key: 'product_id', label: 'Product ID', type: 'number' }, { key: 'warehouse_id', label: 'Warehouse ID', type: 'number' }, { key: 'adjustment_type', label: 'Type' }, { key: 'quantity', label: 'Quantity', type: 'number' }, { key: 'unit_cost', label: 'Unit Cost', type: 'money' }, { key: 'reason', label: 'Reason' }], defaultRow: { product_id: null, warehouse_id: null, adjustment_type: 'increase', quantity: 1, unit_cost: 0, reason: '' } },
    actions: [{ key: 'approve', label: 'Approve', endpointSuffix: 'approve', method: 'patch', permission: 'inventory.adjustments.approve', visibleStatuses: ['draft'], variant: 'secondary' }, { key: 'post', label: 'Post', endpointSuffix: 'post', method: 'patch', permission: 'inventory.adjustments.post', visibleStatuses: ['approved'], variant: 'primary' }, { key: 'void', label: 'Void', endpointSuffix: 'void', method: 'patch', permission: 'inventory.adjustments.void', visibleStatuses: ['posted'], variant: 'danger' }],
  },
