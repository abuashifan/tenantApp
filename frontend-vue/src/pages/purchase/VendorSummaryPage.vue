  ...
  <section class="space-y-5">
    <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-950">AP Vendor Summary</h1>
          <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
            Posted accounts payable balance by vendor with drilldown to vendor ledger detail.
          </p>
        </div>
    ...
    <div class="grid gap-4 md:grid-cols-3">
      <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-xs font-black uppercase tracking-wide text-slate-400">Payable Decrease</p>
        <p class="mt-2 text-xl font-black text-slate-950">{{ formatMoney(totals.debit) }}</p>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-xs font-black uppercase tracking-wide text-slate-400">Payable Increase</p>
        <p class="mt-2 text-xl font-black text-slate-950">{{ formatMoney(totals.credit) }}</p>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-xs font-black uppercase tracking-wide text-slate-400">AP Balance</p>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <label class="text-sm font-bold text-slate-700">
        Search vendor
        <input v-model="search" class="mt-2 h-10 w-full max-w-md rounded-xl border border-slate-200 px-3 text-sm" placeholder="Vendor name or ID" />
      </label>
    </div>

    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div class="border-b border-slate-100 px-5 py-4">
        <h2 class="text-lg font-black text-slate-950">Vendors</h2>
        <p class="mt-1 text-sm text-slate-500">{{ filteredRows.length }} row(s)</p>
      </div>
      <div v-if="loading" class="p-6 text-sm font-bold text-slate-500">Loading vendor summary...</div>
