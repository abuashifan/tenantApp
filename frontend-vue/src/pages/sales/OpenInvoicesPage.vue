  ...
  <section class="space-y-5">
    <div class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h1 class="mt-2 text-3xl font-black tracking-tight text-slate-950">Open Invoices</h1>
          <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
            Posted invoices with outstanding balance and drilldown to invoice or customer ledger detail.
          </p>
        </div>
    ...
    <div class="grid gap-4 md:grid-cols-2">
      <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-xs font-black uppercase tracking-wide text-slate-400">Open Invoices</p>
        <p class="mt-2 text-xl font-black text-slate-950">{{ rows.length }}</p>
      </div>
      <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <p class="text-xs font-black uppercase tracking-wide text-slate-400">Outstanding AR</p>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <label class="text-sm font-bold text-slate-700">
        ...
        <input v-model="search" class="mt-2 h-10 w-full max-w-md rounded-xl border border-slate-200 px-3 text-sm" placeholder="Invoice number or customer" />
      </label>
    </div>

    <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div class="border-b border-slate-100 px-5 py-4">
        <h2 class="text-lg font-black text-slate-950">Open Invoice Rows</h2>
        <p class="mt-1 text-sm text-slate-500">{{ filteredRows.length }} row(s)</p>
      </div>
      <div v-if="loading" class="p-6 text-sm font-bold text-slate-500">Loading open invoices...</div>
